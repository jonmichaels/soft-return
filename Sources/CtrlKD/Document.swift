...
public struct Note: Hashable, Sendable {
    ...
    /// TAGGED footnote/endnote (`tag` set instead — ruling 2026-08-24 item 4: a mark
    /// has no numeric identity to renumber).
    public var number: Int?
    /// Annotations ALWAYS carry this: the nested tag sequence's display TEXT (e.g.
    /// `"AC1"`). Footnote/endnote SOMETIMES carry one too — a user-supplied mark
    /// instead of a number (WSFORMAT.TXT's high-bit-on-high-bit case, ruling
    /// 2026-08-24 item 4). UNTESTED AGAINST A REAL DOCUMENT for footnote/endnote: no
    /// archive specimen carries one; synthetic-fixture-verified only. Comments carry
    /// neither. A tagged note's display MUST use this and must never be renumbered.
    ...
    /// PARSED but NOT honoured for display — `EmitNotes.swift`'s `formatNoteNumber`
    /// (ruling 2026-08-24 item 5) briefly rendered footnote/endnote labels per this
    /// field, but `DISPLAY.WS` printed through real WordStar 7 showed plain arabic for
    /// both a numberFormat=2 footnote and a numberFormat=1 endnote — WordStar does not
    /// use these bits for the printed label. The field stays parsed (preserve-what-
    /// you-find governs the IR) but is simply not consulted for display any more.
...
public enum Provenance: String, Hashable, Sendable {
    case file
    case `default`
    // A --page-settings/preset override: declared (so every `!= .default` gate treats it
    // like a document's own dot command — emit.py:2690's rule) but NOT the document's own
    // `.mt` (so PDFWriter's hm gate, which requires exactly `.file` per pdf.py:2876,
    // correctly skips it). Python's third string, core.py:4287.
    case machineDefault = "machine-default"
}
...
public struct PageGeometry: Hashable, Sendable {
    ...
    /// True when ANY line's `.po` differs from `poCols` above — same "one flag" shape as
    /// `lhVaries`, for the same stateful reason (register b31): `poCols` is the document
    /// DEFAULT (the file's first occurrence) and `Line.poCols` carries the rest. LJ6DTP.WS
    /// moves `.po` five times.
    public var poVaries: Bool
...
public struct Document: Hashable, Sendable {
    ...
    /// `.PSB`'s numeric argument, in LINES, first occurrence wins — WordTsar's own
    /// invented paragraph-spacing-before extension. `nil` unless `producer ==
    /// "wordtsar"` AND a numeric `.PSB` argument was actually seen. Best-effort: no
    /// per-block paragraph-spacing model exists, so this is ONE document-wide value
    /// applied uniformly (Printed/Native RTF's own `\sb`, round 6).
    public var spaceBeforeLines: Double?
    /// `.PSA`'s numeric argument, in LINES — the same producer-signal/first-occurrence
    /// rules as `spaceBeforeLines`, Printed/Native RTF's own `\sa`.
    public var spaceAfterLines: Double?
    ...
    /// Raw PCL printer-payload bytes from 0x0F user print controls, in document order —
    /// indexed by a span's own `pcl` field (same mechanism as `graphics`/`pix`). Register
    ...
    /// The `fonts` INDEX in force at each line's own `.h#`/`.f#` definition — absent when
    /// that definition carried no font-change block of its own (register C6: LJ6DTP's
    /// `.h1` opens with a type-2 Font block, Antique Olive proportional, that a renderer
    /// must resolve the same way a body span's own font run is; PRINTED mode fell back to
    /// a hardcoded Courier because nothing captured it). Final state, the same limitation
    /// `headers`/`footers` have — a document that redefines a running head's FONT
    ...
    public init(
        ...
        producer: String? = nil,
        spaceBeforeLines: Double? = nil,
        spaceAfterLines: Double? = nil,
        ...
        self.spaceAfterLines = spaceAfterLines
