...
public enum StructuralMark: Hashable, Sendable {
    ...
    /// `Span.pctlHMI`.
    ///
    /// `pcl` (register C2, LJ6DTP parity) is the index into `Document.pclPrograms` of
    /// this control's RAW PRINTER PAYLOAD — the bytes WSFORMAT.TXT says go "directly to
    /// the printer" — or `nil` for a display-only control that carries none. LJ6DTP.WS
    /// embeds its page border (all 8 pages) and page 4's checkerboard entirely as raw
    /// PCL in that payload; a renderer that wants to draw them needs the bytes
    /// themselves, not the screen-only description. Same index-by-mark mechanism `pix`
    /// already uses for `Document.graphics`.
    case pctl(hmi: Int, byteLen: Int, pcl: Int?)
    /// b24 round 19 (RULINGS-LEDGER PIX row): an INSET graphic placeholder (0x10),
    /// carrying its index into `Document.graphics` and its placeholder text's byte
    /// length in the cleaned stream — same mechanism as `pctl` above. A renderer that
    /// wants to embed the real image finds both the span (to replace) and the path (to
    ...
    /// sits when the tab fires: MEASURED against real WS7 PCL (LJ6DTP.pcl page 5) -- two
    /// table rows whose tabs fire at different pen positions (after "Black" in one, after
    /// a dot-leader run in the other) both carry the SAME `content[2:4]` (4680 HMI), and
    /// only reading it as absolute-from-margin reproduces both rows' real bar position;
    /// reading it as relative-from-pen would put the second bar 772 HMI (0.43in) further
    /// right than WS7 actually printed it. Never read anywhere before this.
    ///
    /// The placeholder text (`leader * cols`, from `content[0:2]`) is unchanged and keeps
    /// every non-positioning consumer (text/RTF/HTML, Show Invisibles, `tabAt`'s reflow
    /// classifier) working exactly as before -- only a Printed-mode PDF span need ever
    /// consult this mark. See `Span.tabHMI`.
    case tab(absHMI: Int, leader: UInt8, cols: Int)
}
...
public struct SymmetricBlocksResult: Hashable, Sendable {
    ...
    /// Raw PCL printer-payload bytes from 0x0F user print controls that carry them, in
    /// document order — indexed by a `pctl` mark's own `pcl` field (same mechanism as
    /// `graphics`/`pix`). Register C2 (LJ6DTP parity): the border on all 8 pages and
    /// page 4's checkerboard are both raw PCL a renderer EXECUTES rather than draws
    /// generically. This payload used to be computed only to search it for a `%F"NAME"`
    /// file reference and then discarded.
    public let pclPrograms: [[UInt8]]
...
/// Offset of the closing `0x1D` of the WS5+ symmetric block starting at `i`, or `-1`
/// when the framing does not close. Port of `_block_end` (core.py).
///
/// THE one definition of "this is a real symmetric block". A WS5+ sequence is
///
///     1D <count:u16 LE> <cmd> <payload> <count:u16 LE> 1D
///
/// with the leading and trailing counts EQUAL and the closing bracket actually there;
/// `count` spans from the cmd byte through the echoed count, so it is
/// `payload.count + 4` and can never be below 4. The spec says a bare `0x1D` "should
/// not appear in files"; WordStar itself TRUNCATES a file when fooled by one
/// (engineering note 650, "false symmetrical sequences"). `symmetricBlocks` walks with
/// this predicate and `detect` counts with it — one notion of validity, not two.
func symmetricBlockEnd(_ data: [UInt8], _ i: Int) -> Int {
    guard i + 3 <= data.count else { return -1 }
    let jump = Int(data[i + 1]) | (Int(data[i + 2]) << 8)   // little-endian 16-bit
    let end = i + 2 + jump
    guard jump >= 4, end < data.count, data[end] == 0x1d,
          (Int(data[end - 2]) | (Int(data[end - 1]) << 8)) == jump else { return -1 }
    return end
}

/// How many WELL-FORMED `0x1D` symmetric blocks `data` contains. Port of
/// `_count_symmetric_blocks` (core.py).
///
/// Counting bare `0x1D` BYTES instead was the b28 defect (job-493): byte `0x1D` turns
/// up by chance about once per 256 bytes, so a 2 MB Windows executable carries
/// thousands of them — dosbox-x.exe had 5,990 — and `detect`'s "1D blocks are WS5+
/// machinery regardless of anything else" rule then accepted a 21 MB .exe as a
/// WordStar document and rendered it to a 64 MB PDF. Random bytes essentially never
/// satisfy `symmetricBlockEnd`'s framing, so the structural count of that same file is 0.
///
/// Walks exactly as `symmetricBlocks` does — `<1B x>` is an extended-character escape
/// whose second byte is DATA (a `0x1D` there starts nothing), and a block that closes
/// is consumed whole. Python skips between the interesting bytes with a compiled
/// `\x1b.|\x1d` scan because it runs on multi-megabyte files; a plain Swift loop over
/// the bytes is already that scan, and matches it byte for byte: a trailing lone
/// `0x1B` with no data byte after it matches neither alternative there, and is
/// likewise stepped over here.
func countSymmetricBlocks(_ data: [UInt8]) -> Int {
    var n = 0
    var i = 0
    while i < data.count {
        ...
        if data[i] == 0x1d {
            let end = symmetricBlockEnd(data, i)
            if end >= 0 {
                n += 1
                i = end + 1
                continue
...
public func symmetricBlocks(_ data: [UInt8]) -> SymmetricBlocksResult {
    ...
    // Register C2: raw PCL printer payloads, in document order — see `pclPrograms`.
    ...
    while i < data.count {
        ...
        if data[i] == 0x1d && i + 3 <= data.count {
            ...
            // and the closing bracket must be there (`symmetricBlockEnd`, the single
            // definition `detect` also counts with).
            //
            ...
            let jump = end - i - 2              // little-endian 16-bit count, revalidated
            ...
            if let kind = noteKind(forCmd: cmd) {
                ...
                // sentinel byte. A3.
                let content = blockContent(block)
                let (cols, leader) = tabColumns(content)
                tabAt.insert(out.count)
                // The REAL positioning fact travels separately, as a MARK (the same
                // mechanism fnref/pctl/colour already use) -- see `StructuralMark.tab`.
                // The `>= 5` guard is the one `tabColumns` itself uses for its real
                // (non-fallback) path: a malformed block has no real absolute position to
                // attach.
                if content.count >= 5 {
                    let absHMI = Int(content[2]) | (Int(content[3]) << 8)
                    marks[out.count, default: []].append(
                        .tab(absHMI: absHMI, leader: leader, cols: cols))
                }
                ...
                if content.count >= 3 {
                    ...
                    if !name.isEmpty {
                        ...
                        // The raw printer bytes, if any survive the file-reference strip
                        // above — recorded by INDEX (not inline in the mark) so
                        // `decodeSpans` can tag the span with it exactly like a pix
                        // placeholder tags `graphics`. `nil` when there is no printer
                        // payload at all (a display-only control). Register C2.
                        var pclIdx: Int? = nil
                        let printer = Array(content[split...])
                        if !printer.isEmpty {
                            pclIdx = pclPrograms.count
                            pclPrograms.append(printer)
                        }
                        marks[out.count, default: []].append(
                            .pctl(hmi: hmi, byteLen: shown.count, pcl: pclIdx))
                        out += shown
                        handled = true
                    }
                }
                ...
                let idx = graphics.count
                graphics.append(path)
                let name = path.split(whereSeparator: { $0 == "\\" || $0 == "/" }).last
                    .map(String.init) ?? path
                // b24 round 19 (RULINGS-LEDGER PIX row): marked exactly like a 0x0F
                // print control's display string (see the pctl mark above) — one span
                // tagged with `idx` so a renderer can find both the placeholder text
                // and, via `idx`, the resolved .PIX path in `Document.graphics`.
                let placeholder = Array("[image: \(name)]".utf8)
...
private func parseNote(kind: NoteKind, cmd: Int, content: [UInt8], offset: Int) -> Note {
        ...
        if remainder[i] == 0x1d && i + 3 <= remainder.count {
            ...
            if innerCmd == cmd {                                   // the internal tag sequence
                ...
                if numeric && innerContent.count >= 5 {
                    // WSFORMAT.TXT, ruling 2026-08-24 item 4: this is the SAME high-bit
                    // rule applied one level deeper ("currently only one level of this
                    // recursion is used" -- so there is no third level to worry about).
                    // The inner block has the identical 5-byte shape as the outer one
                    // (line-count, tag-word, conv-flag): if ITS OWN tag word also has
                    // the high bit set, this note carries a user-supplied MARK (a
                    // "tag") rather than a resolved number -- read at the same offset
                    // (byte 5 on) an annotation's tag is read from, since that is the
                    // one place in this format a note's display text already lives. If
                    // the inner word's high bit is clear, it is a plain override
                    // number -- unit-tested since before this ruling, and unchanged.
                    // UNTESTED AGAINST A REAL DOCUMENT: no archive specimen carries a
                    // footnote/endnote tag; synthetic-fixture-verified only.
                    let innerTagWord = Int(innerContent[2]) | (Int(innerContent[3]) << 8)
                    if (innerTagWord & 0x8000) != 0 {
                        if innerContent.count > 5 {
                        ...
                        // number stays nil: a tagged note has no numeric identity to
                        // renumber (never overwrite a MARK).
                    } else {
                        number = innerTagWord
    ...
    if kind == .annotation || tag != nil {
        // spec: "Byte: Conversion flag. Not used for annotations" -- and, by the same
        // words, "used only when there is no internal tag": a footnote/endnote
        // carrying a resolved TAG (ruling 2026-08-24 item 4, above) is in exactly that
        // case, so its conv_flag byte (the OUTER one -- a tagged note's inner block
        // never reassigns convFlag) reports no format/convert-to noise either.
        numberFormat = 0
        convertTo = 0
    } else {
