...
struct FormatState {
    ...
    ///
    /// Also read (first occurrence only) by `parsePageDot` into the page geometry, which is
    /// the DOCUMENT-LEVEL default — `PDFLayout.swift`'s `printedLeft`, `--diagnose`. That
    /// reading is not wrong; it is incomplete, the SAME gap register C24 already named for
    /// `.lh` above: `.po` is stateful exactly like every other command in this struct, and
    /// LJ6DTP.WS sets it FIVE times — once for its ordinary .7in body margin, once (2.5")
    /// for page 4's racing-stripe checkerboard. Carried per LINE (`Line.poCols`) for the
    /// same reason `.lh` is: the left origin is a property of where a line's own text
    /// starts, not of the document as a whole — which is also why it is NOT part of
    /// `blockFormat` and therefore never closes a block.
...
func applyFormatDot(_ cmd: [UInt8], _ state: inout FormatState) {
    ...
    case "PO":
        ...
        // `resolveColsArg` never rejects (unlike `.lh`'s `resolveLhArg`), so this is a
        // direct assignment, matching `.lm`/`.rm`/`.pm` below and `parsePageDot`'s own
    ...
    case "LM", "RM", "PM":
        ...
        let nameUpper = String(decoding: name.map(asciiUpper), as: UTF8.self)
        let isLMOrPM = nameUpper == "LM" || nameUpper == "PM"
        if isLMOrPM, unit == nil || unit!.isEmpty {
            ...
            // margins).
            // `.pm` shares the SAME absolute column frame as `.lm`/`.po` (EmitRTF's
            // `rtfPMFiTwips` doc comment) and is therefore ALSO 1-based -- missed when
            // this gate was first written because nothing consumed paraMargin yet.
            // Dormant until the b24 wave (PDFLayout's `printedPMFiPt`, EmitRTF's
            // `rtfPMFiTwips`) started reading it; without this, a `.pm`-bearing block's
            // first line sits one column right of the file's own bytes. Register (b26).
