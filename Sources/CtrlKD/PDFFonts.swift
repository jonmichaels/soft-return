...
///      approximate;
///   2. THE PROPORTIONAL BIT, decisive (ctrl-kd round 9, Jon's ruling, tier-1 evidence):
///      `entry.proportional == false` -> Courier, full stop, REGARDLESS of the
///      typestyle's own name. This is the record's own declared pitch, not a name-based
///      guess -- WSFORMAT's generic Non-PostScript typestyles ("NPS SansSer Qual"/"NPS
///      Serif Qual" and friends) are letter-quality dot-matrix categories, not real
///      PostScript serif/sans faces, and a document can declare proportional=false for
///      ANY typestyle name, mono-sounding or not. Promoting one of these to Times/
///      Helvetica was Jon's field-reviewed "crazy fat" defect: wrong weight (a full
///      commercial proportional face reads heavier than the era's NLQ approximation)
///      AND wrong advance widths (the existing HMI/Tz grid machinery already renders
///      proportional=false content at its own true pitch -- Courier is the only base-14
///      family that grid can be honest at);
///   3. fixed-pitch NAMES -> Courier (see `monoFamilies`) -- tier-2, for a record whose
///      own proportional bit is unavailable rather than false (fontless entries hit the
///      `nil` return above instead, so this tier is for anything else still short a
///      clean bit -- WS4-era);
///   4. the font block's own generic-style bits: serif -> Times, sans -> Helvetica.
...
///   5. no font at all -> Courier, the emitter's own default.
///
/// Bold and italic are NEVER decided here — they come from the span's own b/i styles, exactly
/// as they always have (a proportional=false record that's ALSO span-bold still renders
/// Courier-Bold, never Times-Bold).
///
/// Python has a further case — an entry whose `generic_style` key is missing — which cannot
...
///
/// NOT routed through `FontMap.swift`'s `resolveFont` (b24 round 21 item 5, the shared
/// target-agnostic resolution PDF/RTF/HTML now share where their algorithms actually
/// agree): this function's own SHAPE is a one-of-5 base-14 SELECTION with its own
/// tier-2 fixed-pitch-name check (`monoFamilies`, deliberately narrower than
/// `fontAlternates`' own broader "suggests Courier New" set — see that constant's own
...
func pdfFamily(_ entry: FontChange?) -> PDFFamily {
    ...
    if !entry.proportional { return .courier }
