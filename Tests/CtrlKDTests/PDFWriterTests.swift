...
@Test func printedPDFOfAContentFreeDocumentStillHonoursItsOwnMarginsAndHeader() {
    // REF/ADVANCE.DOT / REF/GALLEYS.DOT (Sawyer WS7 archive): two WordStar "pseudogalley"
    // template documents that declare page geometry and a running header/footer but
    // contain NO body text at all. `layoutPrintedPagesPlain` never appends a single
    // PageLine for either (there is nothing to append), so its trailing
    // `if !page.isEmpty { closePage() }` never fires, and `docToPagelines` falls through
    // to `finalizePages`'s `pages.isEmpty` branch -- Python's own `pages or [[]]`.
    // ctrl-kd's fallback there is a bare Python LIST (no `.headers` attribute at all), so
    // `getattr(pl, 'headers', None)` returns None and `_running_ops` falls back to
    // `doc.headers`/`doc.footers` (the document's own final state) -- which is how
    // ctrl-kd renders a real header/footer, positioned by the document's OWN `.mt`/`.mb`,
    // even on a document with no body. sr's `Page` struct instead has a concrete
    // (non-optional) `headers`/`footers` dict that defaults to `[:]` even in this
    // fallback, and the call site never passes `nil`, so `runningOps`'s own
    // `headers ?? doc.headers` fallback was dead code: the running head/foot -- the ONLY
    // per-document geometry that would show up on a content-free page -- was silently
    // dropped, so two such documents with entirely different margins produced
    // byte-IDENTICAL PDFs (sha256 7bd8b04c... for both, before this fix).
    func doc(mt: String, mb: String) -> Document {
        parseWS(bytes(".mt \(mt)\r\n.mb \(mb)\r\n.he HEADER-#\r\n.fo FOOTER-#\r\n"))
    }
    let a = doc(mt: ".7i", mb: ".6i")
    let b = doc(mt: "1.25i", mb: "1.50i")
    let pdfA = emitPDF(a, mode: .printed)
    let pdfB = emitPDF(b, mode: .printed)
    #expect(pdfA != pdfB, "different .mt/.mb must not collapse to identical output")
    #expect(contains(pdfA, bytes("HEADER-1")))
    #expect(contains(pdfA, bytes("FOOTER-1")))
}
