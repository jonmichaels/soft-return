...
@Test func modernPDFEndnotesCollectAtDocumentEnd() throws {
    ...
    // endnote i cannot collide. Entry format (`i.`, no brackets) per ruling
    // 2026-08-23/24 item 2.
...
@Test func noteRefsPrefixedSchemeMatchesMarkdownLabels() {
    ...
    // endnote: inline mark, entry (no brackets -- ruling 2026-08-24 item 2)
    #expect(texts.contains("e1") && texts.contains("e1."))
    #expect(texts.contains("a1") && texts.contains("[a1]"))   // annotation: unchanged
...
@Test func runningHeadToggleBytesBecomeStylesNotGlyphs() throws {
    ...
    var data = ws7Block(0x00)
    data += bytes(".he ") + [0x02] + bytes("Big Bold Header")
    data += [0x02] + bytes(" / #") + HARD
...
@Test func modernFlowIsThePublicSemanticContract() throws {
    ...
    var paras: [(align: Alignment, indentCols: PyNumber, runs: [SemanticRun],
                 footnotes: [SemanticFootnote])] = []
    for item in sem.items {
        if case .para(let align, let indentCols, _, let runs, let footnotes, _, _, _) = item {
    ...
    let quote = try #require(paras.first { $0.indentCols.value == 7 })   // M2 margins
