...
/// A footnote/endnote carrying a user-supplied MARK instead of a number (ruling
/// 2026-08-24 item 4): the SAME high-bit-nested-sequence shape `ws7Note` exercises for a
/// plain NUMBER override, but with the INNER block's OWN tag word ALSO high-bit-set —
/// WSFORMAT.TXT's rule applied one level deeper, since "currently only one level of this
/// recursion is used" leaves no third level. The tag text sits at the same byte offset
/// (5) an annotation's tag already comes from. Direct port of Python's
/// `ws7_note_tagged` test helper (`test_note_rulings_20260824.py`).
func ws7NoteTagged(
    cmd: UInt8, text: [UInt8], tagText: [UInt8], lineCount: Int = 1, junkConvFlag: UInt8 = 0x00
) -> [UInt8] {
    let innerContent: [UInt8] = [0x00, 0x00, 0x00, 0x80, junkConvFlag] + tagText
    let inner = ws7Block(cmd, payload: innerContent)
    let split = text.count / 2
    let outerText = Array(text[..<split]) + inner + Array(text[split...])
    let content: [UInt8] = [
        UInt8(lineCount & 0xFF), UInt8((lineCount >> 8) & 0xFF), 0x00, 0x80, junkConvFlag,
    ] + outerText
    return ws7Block(cmd, payload: content)
}
...
func styleRecord(left: Int = 1800, tabs: [Int] = [900, 1800], decimalTabs: Int = 0,
    ...
    if let font {
        put(0, le16(font.width) + le16(font.height) + le16(font.typestyle))
    } else {
        put(0, le16(0xFFFF))                      // font: inherited
    }
    put(10, le16(left))                           // left margin HMI
    put(12, le16(right ?? 0xFFFE))                // right: given, else inherited
...
/// One WS5+ colour block (symmetrical sequence 0x01): current colour byte, previous
/// colour byte — the CGA/EGA palette index (0-15), per WSFORMAT.TXT type 1 Color. Port
/// of Python's `_colour_block` test helper (b24 round 18).
func colourBlock(_ colour: Int, previous: Int = 0) -> [UInt8] {
    ws7Block(0x01, payload: [UInt8(colour), UInt8(previous)])
}
