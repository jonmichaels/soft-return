...
/// is proven separately by `notesVectors1_2_0` (a cross-engine recording from ctrl-kd's own
/// fixed implementation, not real WordStar 7) and `ws7FootnoteExtractionAndRef`
/// (ParseWSTests.swift, using the
...
@Test func symmetricBlocksMatchesPythonVectors() throws {
    ...
    // Real note-TEXT extraction is covered by `notesVectors1_2_0` below, against a
    // cross-engine recording (ctrl-kd) generated by the Python engine's own fixed
    // implementation -- not real WordStar 7.
...
@Test func parseWSMatchesPythonVectors() throws {
    ...
    for (i, v) in vectors.enumerated() {
        ...
        // against a cross-engine recording (ctrl-kd) from the Python engine's own fixed
        // implementation, not real WordStar 7; what's still
...
@Test func pdfConstantsMatchPython() throws {
    ...
    // this vector file to drop the now-nonexistent Python constant would erase a real
    // cross-engine recording (ctrl-kd) for the nine that remain, so it stays as captured;
    // the Swift side just
...
/// with the transformation from the old cross-engine recording (ctrl-kd) to the new one.
///
...
/// this out: an honest recording of Python's OWN output, misleading label). 1.1.6 pops that page, so the case is now
/// `[53]`.
///
/// The job-012 JSON is left exactly as Athena generated it, because it is a correct record of
/// what 1.1.5 did and rewriting the recording to match a later version would destroy
...
@Test func emitPDFMatchesPythonVectorsByteForByte() throws {
    ...
    // Courier-grid answer.
    //
    // `printed_stream`'s `expected_hex` was RE-TAKEN a second time (2026-09-07, against the
    // live Python reference directly, same as `modern_ws4` above): this printstream sets
    // none of `.pn`/`.pg`/`.op`, so `pgnumCheckpoints`'s corrected default (ws7-prints/v3
    // finding #2, seeded ON to match stock WS7, ported from ctrl-kd b6d5d03) now adds a
    // stock automatic page number to both of its pages — a real, evidenced, deliberate
