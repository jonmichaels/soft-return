...
@Test func alignmentActuallyRendersInEveryFormatThatCanShowIt() {
    ...
    // b24 round 20 (slate item 4): a centered paragraph now also carries the tight
    // verse line-height on the SAME style attribute.
    #expect(html.contains(#"<p style="text-align:center;line-height:1.15">"#))
...
@Test func marginsArePerBlockStateNotFirstOccurrence() {
    ...
    // `.pm` shares the same 1-based column frame (b26 fix) so `.pm 4` -> 3.0 offset
    // columns, same normalization as `.lm`.
    #expect(doc.blocks.map { [$0.leftMargin, $0.rightMargin, $0.paraMargin] }
            == [[4.0, 60.0, nil], [4.0, 60.0, 3.0]])
