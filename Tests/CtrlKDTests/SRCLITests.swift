...
/// A minimal `CLIEnvironment` for `diagnose()` calls in this file that never exercise
/// PIX resolution (no `doc.graphics` in these fixtures) -- every filesystem probe
/// answers "nothing here", matching `CLIEnvironment`'s own documented defaults.
/// `PicturesTests.swift` exercises the real resolve/report/export path.
private func noFSEnvironment() -> CLIEnvironment {
    CLIEnvironment(readFile: { _ in [] }, writeFile: { _, _ in }, createDirectory: { _ in },
...
@Test func diagnoseRendersSortedKeysAndTwoSpaceIndent() {
    let rendered = diagnose(path: "/tmp/printstream", data: bytes("Line one\r\nLine two\r\n"),
                           environment: noFSEnvironment()).render()
    #expect(
        rendered == """
            {
...
@Test func diagnoseNotesObjectCountsEachKindSeparately() throws {
    let value = diagnose(path: "/tmp/four", data: fourKindData(), environment: noFSEnvironment())
...
@Test func diagnoseNotesObjectIsAllZeroesWhenThereAreNoNotes() throws {
    let value = diagnose(path: "/tmp/plain", data: makeProse(), environment: noFSEnvironment())
    let fields = try #require(normalize(value).object)
...
@Test func diagnoseReportsResolvedPageGeometryWithProvenance() throws {
    ...
    let value = diagnose(path: "/tmp/legal", data: data, environment: noFSEnvironment())
    let fields = try #require(normalize(value).object)
...
@Test func diagnoseReportsDefaultPageGeometryWhenNothingOverridesIt() throws {
    let value = diagnose(path: "/tmp/plain", data: fourKindData(), environment: noFSEnvironment())
    let fields = try #require(normalize(value).object)
    let page = try #require(fields["page"]?.object)
...
@Test func diagnoseReportsProducerOnlyWhenWordTsarCommandsAreSeen() throws {
    ...
    let value2 = diagnose(path: "/tmp/plain", data: without, environment: noFSEnvironment())
...
@Test func diagnoseReportsCommentBugSignatureForPrintstreamFilesOnly() throws {
    ...
    let value = diagnose(path: "/tmp/bug", data: buggy, environment: noFSEnvironment())
    let fields = try #require(normalize(value).object)
    ...
    let cleanFields = try #require(normalize(diagnose(path: "/tmp/clean", data: clean,
                                                     environment: noFSEnvironment())).object)
    ...
    let ws5Fields = try #require(normalize(diagnose(path: "/tmp/ws5", data: fourKindData(),
                                                   environment: noFSEnvironment())).object)
...
/// b33 N9 (mirrored from ctrl-kd 0750948, "same auto/on/off shape as --page-numbers"):
/// `--sentence-spacing {auto,keep,single}`, default `.auto` on a bare run, choices
/// validated the same way `--mode`/`--variant` are above.
@Test func sentenceSpacingFlagIsValidatedAgainstItsChoices() {
    guard case .run(let bare) = parseArguments(["A.WS"]) else {
        Issue.record("expected a run")
        return
    }
    #expect(bare.sentenceSpacing == .auto, "the bare CLI default must be auto")

    guard case .run(let keep) = parseArguments(["--sentence-spacing", "keep", "A.WS"]) else {
        Issue.record("expected a run")
        return
    }
    #expect(keep.sentenceSpacing == .keep)

    guard case .run(let single) = parseArguments(["--sentence-spacing", "single", "A.WS"]) else {
        Issue.record("expected a run")
        return
    }
    #expect(single.sentenceSpacing == .single)

    guard case .usageError(let message) = parseArguments(["--sentence-spacing", "sideways", "A.WS"]) else {
        Issue.record("expected a usage error")
        return
    }
    #expect(message.contains("invalid choice: 'sideways'"))
    #expect(message.contains("'auto', 'keep', 'single'"))
}
...
@Test func versionLineNamesBothTheCLIAndTheReference() {
    ...
    #expect(versionLine(devDate: srDevDate) == "sr v4.0.0 (ctrl-kd parity 4.0.0)")
    #expect(versionLine(devDate: "2026-08-14") == "sr v4.0.0 (dev 2026-08-14) (ctrl-kd parity 4.0.0)")

    let recorder = Recorder()
    #expect(run(["--version"], environment: recorder.environment) == ExitStatus.ok)
    #expect(recorder.out == [versionOutput])
    #expect(versionOutput.hasSuffix("sr v4.0.0 (ctrl-kd parity 4.0.0)"))
    #expect(versionOutput.contains("_____       ______     ____"))  // the SOFT RETURN Slant banner leads
    #expect(recorder.written.isEmpty)

    // --verbose beside --version: a no-op without a stamp (this repo's committed state) —
    // same output, no error, either flag order.
    let verboseRecorder = Recorder()
    #expect(run(["--version", "--verbose"], environment: verboseRecorder.environment) == ExitStatus.ok)
    #expect(verboseRecorder.out == [versionOutput])
    let reversed = Recorder()
    #expect(run(["--verbose", "--version"], environment: reversed.environment) == ExitStatus.ok)
    #expect(reversed.out == [versionOutput])
}
...
/// b33 N9 (mirrored from ctrl-kd's `test_cli_sentence_spacing_flag_forces_keep_on_modern_default`):
/// what only the CLI can prove is that `--sentence-spacing` actually reaches the emitter
/// through `run`'s conversion loop, same doctrine as the note-flag wiring tests above.
@Test func sentenceSpacingFlagReachesTheEmitterAndForcesKeepOnModernDefault() throws {
    let data = ws7Block(0x00) + bytes("He said hello.  She replied yes.") + HARD
    let recorder = Recorder(files: ["/in/SAMPLE.WS": data])
    // bare -- mode defaults to modern, whose own default (auto) collapses to single
    #expect(run(["-t", "text", "/in/SAMPLE.WS"], environment: recorder.environment) == ExitStatus.ok)
    let auto = String(decoding: try #require(recorder.written["/in/SAMPLE.txt"]), as: UTF8.self)
    #expect(auto.contains("hello. She"))

    // --force: reusing the same output path is a legitimate overwrite here, not what
    // the D4 prompt/refusal (ruling 2026-08-05) exists to catch.
    #expect(run(["--sentence-spacing", "keep", "--force", "-t", "text", "/in/SAMPLE.WS"],
                environment: recorder.environment) == ExitStatus.ok)
    let kept = String(decoding: try #require(recorder.written["/in/SAMPLE.txt"]), as: UTF8.self)
    #expect(kept.contains("hello.  She"))
}
...
@Test func diagnoseReportsParseEvidenceForWS5Files() throws {
    ...
    let value = diagnose(path: "/tmp/ws7", data: data, environment: noFSEnvironment())
    let fields = try #require(normalize(value).object)
...
@Test func diagnoseCountsParagraphBlocksOnly() throws {
    ...
    let fields = try #require(normalize(diagnose(path: "/tmp/pa", data: data,
                                                environment: noFSEnvironment())).object)
