...
///
/// PATH SCRUBBED (planning #189, Jon: "remove the directory but verify that the test still
/// works"). The captured event carried a real folder path from Jon's own machine, in three
/// separate encodings inside the flattened bytes: the sandbox-extension token's trailing
/// path, an HFS colon-delimited alias path, and a POSIX path. All three now name a fake
/// directory of the SAME SHAPE, and the document basename with them.
///
/// The substitution is byte-length-preserving, every segment swapped for one of identical
/// length, so the flattened descriptor's own length-prefixed fields stay valid and the bytes
/// still unflatten to the same descriptor tree they always did — which is precisely what
/// `realFixtureDecodesToTheRealConvertEvent` / `realFixtureReplyRequestedAttribute` /
/// `realFixtureAttributesSurviveTheUnflatten` assert, and why they are the proof that this
/// scrub did not quietly turn the fixture into an inert blob. The one assertion that named
/// the old basename now names the new one; nothing else about the replay changed.
///
/// The leading 64-hex sandbox-extension digest is KEPT: it is a hash, not a path, and it is
/// what makes the token structurally realistic. It no longer corresponds to the path beside
/// it, which costs this suite nothing — the token was never re-validatable off Jon's machine
/// anyway, and no test asserts on the digest.
...
extension ConvertCommandReceiverDispatchTests {
    ...
    /// Planning #191 (incident): the real fixture's own `keyDirectObject` is a file alias to a
    /// real file under Jon's home, and the fixture carries NO `'SRcf'` param — so replaying it
    /// unmodified through the installed handler makes `BesideSourceWriter` write an RTF beside
    /// that real file (exactly what happened). BEFORE either replay test below hands the
    /// fixture's bytes to the handler, this swaps in a bundled sample (copied into a
    /// throwaway `sampleDir`) for `keyDirectObject` and adds an explicit `'SRcf'` pointing at
    /// a separate throwaway `destDir` — the fixture never carried one. Every attribute this
    /// file's investigation actually cares about (`subj`/`csig`/`shas`/`addr`/`from`) rides
    /// along untouched; only the two params that decide WHAT gets read and WHERE the output
    /// lands are replaced. Callers must remove both returned directories.
    @MainActor
    private static func redirectToASafeSampleCopy(
        _ event: NSAppleEventDescriptor, label: String
    ) throws -> (sampleDir: URL, destDir: URL) {
        let sampleDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("ConvertCommandReceiverDispatchTests-\(label)-source-\(UUID().uuidString)")
        let destDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("ConvertCommandReceiverDispatchTests-\(label)-dest-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: sampleDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: destDir, withIntermediateDirectories: true)
        let sampleCopy = try BundledSampleFixture.copy("OCAPTAIN.WS", into: sampleDir)

        let fileList = NSAppleEventDescriptor.list()
        fileList.insert(NSAppleEventDescriptor(fileURL: sampleCopy), at: 0)
        event.setParam(fileList, forKeyword: keyDirectObject)
        event.setParam(NSAppleEventDescriptor(fileURL: destDir), forKeyword: ScriptingCodes.fourCharCode("SRcf"))

        return (sampleDir, destDir)
    }
    ...
    @Test func realFixtureAttributesSurviveTheUnflatten() throws {
        ...
        // planning #189: the captured path is SCRUBBED — see `real-convert-event-b9.base64`'s
        // own note in this suite's doc comment. The assertion still proves the same thing it
        // always did (the sandbox-hash-string survives the unflatten carrying the document
        // path the event was captured for), against the fake path now in the fixture.
        #expect(sbhsString?.lowercased().contains("sample1.ws") == true, "expected the sandbox-hash-string to name the captured file, got \(String(describing: sbhsString))")
    }
    ...
    @Test @MainActor func realFieldEventDoesNotReproduceTheFieldFailureAtLayer2() throws {
        let event = try Self.loadRealFixtureDescriptor()
        let (sampleDir, destDir) = try Self.redirectToASafeSampleCopy(event, label: "layer2")
        defer {
        ...
        let destContents = try FileManager.default.contentsOfDirectory(at: destDir, includingPropertiesForKeys: nil)
        #expect(destContents.contains { $0.pathExtension.lowercased() == "rtf" },
                "expected the redirected RTF to land in the explicit SRcf destination, found \(destContents)")
        let sampleDirContents = try FileManager.default.contentsOfDirectory(at: sampleDir, includingPropertiesForKeys: nil)
        #expect(!sampleDirContents.contains { $0.pathExtension.lowercased() == "rtf" },
                "nothing should ever be written beside the sample copy — planning #191's incident shape, found \(sampleDirContents)")
    }
    ...
    func realisticMimicOfEveryNamedAttributeAlsoDoesNotReproduce() throws {
        ...
        // planning #189: same scrub as the recorded fixture — this hand-built mimic exists to
        // carry the same SHAPE of sandbox-hash-string the real event did (the hash, the flag
        // fields, then a path), and the shape is what the test exercises. The leading hash is
        // the captured one, kept: it is a digest, not a path.
        let realSandboxHashString =
            ...
            + "0000000005af26e4;01;/users/pat/storage/sample_documents/testdisk-ws-image/docs/sample1.ws"
    ...
    @Test @MainActor func realFieldEventReplyResultDescriptorRemainsAbsentEvenAfterTheTypeFix() throws {
        let event = try Self.loadRealFixtureDescriptor()
        let (sampleDir, destDir) = try Self.redirectToASafeSampleCopy(event, label: "reply-inspect")
        defer {
            try? FileManager.default.removeItem(at: sampleDir)
            try? FileManager.default.removeItem(at: destDir)
        }
