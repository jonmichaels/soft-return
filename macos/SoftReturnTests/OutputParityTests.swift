...
/// docstring: "Runs on the engine host where ctrl-kd and the Sawyer tree live"). A full v10 (all 151
/// files) is an out-of-repo job.
///
/// **v11 (job 426, FULL RESCOPE):** generated on the engine host (`ctrl-kd b07a9c7`) directly against
...
/// (`CTRLKD_PRIVATE_CORPUS`-gated) and the out-of-repo sweep are no longer covering anything
...
/// drifted from `c1b622d` since job 488) is separate, larger, out-of-scope work for a dev-host
...
/// environment. Tier 2 (env-gated, below) and the out-of-repo sweep are the full-matrix gate.
enum OutputManifest {
    struct CellEntry {
        let sha256: String?
        let bytes: Int?
...
@Suite struct OutputParityTests {
    ...
    /// out-of-repo sweep remain wired but currently have nothing left to cover beyond this suite.
    ///
